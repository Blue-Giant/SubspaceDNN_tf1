...
def solve_Multiscale_PDE(R):
    ...
    if R['PDE_type'] == 'general_laplace':
        ...
        if R['equa_name'] == 'multi_scale2D_7':
            region_lb = -1.0
            region_rt = 1.0
            f = MS_LaplaceEqs.force_side2E6(input_dim, out_dim)                       # f是一个向量
            u_true = MS_LaplaceEqs.true_solution2E6(input_dim, out_dim, eps=epsilon)
            u_left, u_right, u_bottom, u_top = MS_LaplaceEqs.boundary2E6(input_dim, out_dim, region_lb, region_rt,
                                                                         eps=epsilon)
            A_eps = MS_LaplaceEqs.elliptic_coef2E6(input_dim, out_dim, eps=epsilon)
        ...
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if R['variational_loss'] == 1:
                ...
                if R['PDE_type'] == 'general_laplace':
                    ...
                    loss_it_variational2NN = (1.0 / 2) *dUNN_2Norm - tf.multiply(f(X_it, Y_it), U_NN)
                elif R['PDE_type'] == 'Possion_Boltzmann':
                    a_eps = A_eps(X_it, Y_it)                          # * 行 1 列
                    Kappa = kappa(X_it, Y_it)
                    AdUNN_pNorm = a_eps*tf.pow(norm2dU_NN, p)
                    loss_it_variational2NN = (1.0 / p) * (AdUNN_pNorm + Kappa*U_NN*U_NN) - \
                                             tf.multiply(f(X_it, Y_it), U_NN)
                else:
                    a_eps = A_eps(X_it, Y_it)                          # * 行 1 列
                    AdUNN_pNorm = a_eps*tf.pow(norm2dU_NN, p)
                    loss_it_variational2NN = (1.0 / p) * AdUNN_pNorm - tf.multiply(f(X_it, Y_it), U_NN)
                Loss_it2NN = tf.reduce_mean(loss_it_variational2NN) * (region_rt - region_lb) * (region_rt - region_lb)

                if R['wavelet'] == 1:
                    # |Uc*Uf|^2-->0
                    norm2UdU = tf.reshape(tf.reduce_sum(tf.square(tf.multiply(U_NN_Normal, alpha*U_NN_freqs)), axis=-1), shape=[-1, 1])
                    ...
                    U_dot_U = tf.reduce_mean(tf.square(tf.multiply(U_NN_Normal, alpha*U_NN_freqs)), axis=0)
                    dU_dot_dU = tf.multiply(dU_NN_Normal, alpha*dU_NN_freqs)
                    sum2dUdU = tf.reshape(tf.reduce_sum(dU_dot_dU, axis=-1), shape=[-1, 1])
                    # norm2AdUdU = tf.square(tf.multiply(a_eps, sum2dUdU))
                    norm2AdUdU = tf.square(sum2dUdU)
            ...
            loss_bd_square2freqs = tf.square(alpha*ULeft_NN_freqs) + tf.square(alpha*URight_NN_freqs) + \
                                    tf.square(alpha*UBottom_NN_freqs) + tf.square(alpha*UTop_NN_freqs)
            ...
            if R['PDE_type'] == 'general_laplace' or R['PDE_type'] == 'pLaplace_explicit' \
                    or R['PDE_type'] == 'Possion_Boltzmann':
                # 训练上的真解值和训练结果的误差
    ...
    if R['testData_model'] == 'random_generate':
        ...
        if R['PDE_type'] == 'pLaplace2implicit':
            ...
            test_xy_bach = matData2Laplacian.get_randData2Laplace(dim=input_dim, data_path='data2mat')
            size2batch = np.shape(test_xy_bach)[0]
            size2test = int(np.sqrt(size2batch))
            ...
            if i_epoch % 1000 == 0:
                ...
                if R['PDE_type'] == 'general_laplace' or R['PDE_type'] == 'pLaplace_explicit' or \
                    ...
                    u_true2test, utest_nn, utest_normal, utest_freqs = sess.run(
                        [Utrue, U_NN, U_NN_Normal, alpha*U_NN_freqs], feed_dict={XY_it: test_xy_bach, train_opt: train_option})
                else:
                    u_true2test = u_true
                    utest_nn, utest_normal, utest_freqs = sess.run(
                        [U_NN, U_NN_Normal, alpha*U_NN_freqs], feed_dict={XY_it: test_xy_bach, train_opt: train_option})
...
if __name__ == "__main__":
    ...
    # R['actName2Normal'] = 'sin_modify_mexican'

    # R['actName2Scale'] = 'relu'
    # R['actName2Scale']' = leaky_relu'
    # R['actName2Scale'] = 'srelu'
    R['actName2Scale'] = 's2relu'
    # R['actName2Scale'] = 'sin_modify_mexican'
    # R['actName2Scale'] = 'powsin_srelu'
    # R['actName2Scale'] = 'slrelu'
    # R['actName2Scale'] = 'gauss'
    # R['actName2Scale'] = 'metican'
    # R['actName2Scale'] = 'modify_mexican'
    # R['actName2Scale'] = 'elu'
    # R['actName2Scale'] = 'selu'
    # R['actName2Scale'] = 'phi'
