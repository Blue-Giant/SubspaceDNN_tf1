...
def solve_Multiscale_PDE(R):
    ...
    if R['PDE_type'] == 'Possion_Boltzmann':
        ...
        region_lb = 0.0
        region_rt = 1.0
        u_true, f, A_eps = MS_LaplaceEqs.get_infos2pLaplace_3D(
        ...
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if R['variational_loss'] == 1:
                ...
                if R['PDE_type'] == 'general_laplace':
                    ...
                    loss_it_variational_NN = (1.0 / 2) *dUNN_2Norm - tf.multiply(f(X_it, Y_it, Z_it), U_NN)
                    ...
                    loss_it_variational_NN = (1.0 / p) * (AdUNN_pNorm + Kappa*U_NN*U_NN) - \
                                             tf.multiply(f(X_it, Y_it, Z_it), U_NN)
                else:
                    a_eps = A_eps(X_it, Y_it, Z_it)                          # * 行 1 列
                    AdUNN_pNorm = a_eps*tf.pow(norm2dU_NN, p)
                    loss_it_variational_NN = (1.0 / p) * AdUNN_pNorm - tf.multiply(f(X_it, Y_it, Z_it), U_NN)
                ...
                if R['PDE_type'] == 'Possion_Boltzmann':
                    ...
                    ApNorm2dU_NN = a_eps * tf.pow(norm2dU_NN_Normal, p) + a_eps * tf.pow(norm2dU_NN_freqs, p)
                    loss_it_variational2NN = (1.0 / p) * (ApNorm2dU_NN + Kappa*U_NN*U_NN) - \
                                             tf.multiply(f(X_it, Y_it, Z_it), U_NN)
                else:
                    a_eps = A_eps(X_it, Y_it, Z_it)  # * 行 1 列
                    ApNorm2dU_NN = a_eps * tf.pow(norm2dU_NN_Normal, p) + a_eps * tf.pow(norm2dU_NN_freqs, p)
                    loss_it_variational2NN = (1.0 / p) * ApNorm2dU_NN - tf.multiply(f(X_it, Y_it, Z_it), U_NN)
    ...
    if R['testData_model'] == 'random_generate':
        ...
        test_xyz_bach = matData2Laplacian.get_randData2Laplace(dim=input_dim, data_path=mat_data_path)
        saveData.save_testData_or_solus2mat(test_xyz_bach, dataName='testXYZ', outPath=R['FolderName'])
